...
1. 接続の試行が失敗することに注意してください。
    ...
    >**注**: Labの挙動によっては接続できてしまう可能性があります。本来は **接続できない** が正しい状態です。
...
1. 自由ドメイン名検索を使用して、使用されていないドメイン名を確認します。
...
1. 「Cloud Shell」ペインで次のコマンドを実行して、 新しく作成した DNS ゾーンに設定されている「**az104-04-vm0**」 DNS レコード セットの外部名前解決をテストします (プレースホルダー `[Name server 1]` をこの前のタスクでメモした **ネーム サーバー 1** の名前に、プレースホルダー `[domain name]` をこの前のタスクで作成した DNS ドメインに置き換えます)。
...
1. 「Cloud Shell」ペインで次のコマンドを実行して、 新しく作成した DNS ゾーンに設定されている「**az104-04-vm1**」 DNS レコード セットの外部名前解決をテストします (プレースホルダー `[Name server 1]` をこの前のタスクでメモした **ネーム サーバー 1** の名前に、プレースホルダー `[domain name]` をこの前のタスクで作成した DNS ドメインに置き換えます)。
